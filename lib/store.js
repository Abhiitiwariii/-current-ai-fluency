...
import { TRACK_LENGTH } from "./content";
...
const DEFAULT_STATE = {
  ...
  account: null, // §10: created AFTER the aha — { name, email, createdAt, provider? }. provider:"google" when signed in via Supabase Auth; else local-only.
};
...
export function completeDrop(dropId, capabilityId) {
  ...
  return {
    ...
    // v2: all five capability tiers cleared → the AI-Fluency certificate is earned.
    programComplete: capabilities.length >= TRACK_LENGTH,
  };
}
...
// v3.2: hydrate the local account from a Google (Supabase Auth) identity on
// return from the OAuth redirect. Reuses the same `account` shape so the
// certificate / share card / streak-save all just work with a real name/email.
export function createAccountFromGoogle(user) {
  if (!user) return getState().account;
  const meta = user.user_metadata || {};
  const name =
    (meta.full_name || meta.name || meta.given_name || "").trim() || "there";
  const account = {
    name,
...
// v3.2: sign out of the Google session (best-effort) and clear the local account.
