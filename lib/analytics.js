...
import { mpTrack, mpIdentify, mpRegister, mpReset } from "./mixpanel";
...
export function track(event, props = {}) {
  ...
  // Mirror to Mixpanel so the funnel is visible across ALL users, not just this
  // browser (no-op unless NEXT_PUBLIC_MIXPANEL_TOKEN is set). Align Mixpanel's
  // identity to our persistent user_id; register persona once so funnels break
  // down by job. Never throws — the local log above is the source of truth.
  mpIdentify(user_id);
  if (event === "job_selected" && props.job) mpRegister({ job: props.job });
  mpTrack(event, { user_id, session_id, ...props });
...
export function resetEvents() {
  ...
  mpReset(); // fresh Mixpanel identity too, so a demo reset starts clean
  sessionId = uid();
}
