...
// ─────────────────────────────────────────────────────────────────────────────
// YouTube video hook (v2 Q2/Q3) — one curated video PER PERSONA, shown in-screen
// as the motivating hook BEFORE the simulated awe demo.
//
// ⚠ CURATED PICKS — the `yt` ids below are my starting picks. VERIFY each one
// plays & embeds, or REPLACE it: paste your link and set `yt` to the 11-char id
// (the v=XXXXXXXXXXX part of the URL). Everything else keeps working — the
// VideoHook loads a poster first and always offers a "Continue" fallback, so a
// blocked or dead id never dead-ends the flow.
// ─────────────────────────────────────────────────────────────────────────────
export const VIDEO_BY_JOB = {
  marketing: {
    yt: "3MwMII8n1qM",
    start: 15,
    end: 75, // ~60s hook window (tweak per-video if a better segment exists)
