...
function apply(type, a, b) {
  ...
  else if (type === "people_set") mp.people.set(a);
}
...
// Attach a real name/email to the current user's Mixpanel People profile, so the
// Users view shows who they are instead of just the anon id. Called when identity
// is captured (Google sign-in, "Save my progress", feedback email). Only sets the
// fields we actually have. $name/$email are Mixpanel's reserved display props.
export function mpSetPerson({ name, email, method } = {}) {
  const props = {};
  if (name) props.$name = name;
  if (email) props.$email = email;
  if (method) props.signin_method = method;
  if (Object.keys(props).length === 0) return;
  call("people_set", props);
}
